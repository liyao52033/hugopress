...
description: "本文介绍了部署Cloudreve网盘系统的关键步骤。主要内容包括：部署前的准备工作，如手动创建或通过命令生成必要的配置文件、数据库文件和上传目录；运行Docker容器并配置Nginx域名；以及如何进行更新。文章重点扩展说明了如何通过集成支持WOPI协议的服务（如OnlyOffice）来增强文件的在线预览与编辑功能，并给出了具体的后台配置方法和安全警告。"
url: /pages/6a9d42
type: docs
cover: https://cnb.xiaoying.org.cn?random=/pages/ec23a7
weight: 500
---
