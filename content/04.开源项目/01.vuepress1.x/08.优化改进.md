...
url: /pages/6c372a
type: docs
cover: https://cnb.xiaoying.org.cn?random=/pages/7b46b0
weight: 8
description: >-
  本文介绍了VuePress项目优化改进的五个方面：解决打包时因文件过大（超过500KB）导致的Babel警告，可通过提高内存参数或配置.babelrc文件处理；支持侧边栏显示四级标题；将URL中的中英文及连字符替换为六位随机字符；通过安装和配置PWA插件实现更新提示功能，包括弹窗和Service
  Worker；以及解决因文件夹或文件名使用中文而导致的VueRouter路径警告，需修改内部路由文件进行编码处理。
license: true
twikoo: true
footer: false
---
