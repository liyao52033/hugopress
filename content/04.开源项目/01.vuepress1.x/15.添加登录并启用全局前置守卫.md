...
url: /pages/aa8477
type: docs
cover: https://cnb.xiaoying.org.cn?random=/pages/2964ed
weight: 15
description: "本文介绍了为VuePress项目添加登录功能并启用全局前置守卫的步骤。主要内容包括：新增登录页面和路由，添加登录验证逻辑，安装并使用模态对话框插件，在enhanceApp.js中执行路由守卫逻辑。同时，为了弥补enhanceApp.js在页面刷新或直接访问路由时无法触发的不足，还创建了全局组件LoginInfo，确保未登录时能重定向到登录页，最后在主题配置中完成相应设置。"
---
