...
weight: 70
description: >-
  本文介绍了在Coze国际版平台上创建和部署智能体的完整流程。首先，用户需创建智能体并设置其基本人设与回复逻辑。核心在于通过添加插件（如搜索工具）或知识库来扩展智能体的能力，以处理模型本身无法完成的任务，例如专业领域问答或多模态内容理解。文章进一步详细说明了如何构建工作流，特别是利用链接读取插件来解析PDF等文件，使智能体能基于文件内容回答问题。最后，指南涵盖了授权、安装智能体，并将其以Web
  SDK形式嵌入到外部应用（如博客）中的关键步骤。
categories:
  - 工具类
  - 本站插件
author:
