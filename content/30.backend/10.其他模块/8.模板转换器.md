...
type: docs
icon: menu
cover: https://cnb.xiaoying.org.cn?random=/pages/19b618
description: "本文介绍了一款Spring Boot Velocity转换器，它能够将Java、Vue、TS和XML文件转换为Velocity模板，主要用于代码生成，如从实体类生成VO类等。该工具支持多种文件类型的自动检测与转换、自定义包名和模块名替换、批量处理ZIP文件以及自动变量替换。它通过自动配置的核心组件简化了集成和使用过程。"
---
