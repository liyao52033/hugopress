...
url: /pages/3a2ef9
type: docs
cover: https://cnb.xiaoying.org.cn?random=/pages/bb196b
weight: 130
license: true
twikoo: true
footer: false
description: "本文总结了Nginx的常用配置，包括HTTPS设置、GZip压缩静态文件（重点针对HTML、CSS、JS等文本资源可达到70-90%压缩率）、HTTP自动重定向到HTTPS、缓存时间控制及防止直接访问目录。核心要点是精细化缓存策略：让入口HTML不缓存，带Hash的静态资源永久缓存，并建议CDN配置与源站保持一致。"
---
