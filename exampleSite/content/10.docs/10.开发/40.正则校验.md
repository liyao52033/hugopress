...
description: >-
  本文介绍了正则校验的安装与使用方法，重点讲解了如何通过自定义文件扩展名进行校验。文章详细说明了默认可用类型，并指导用户如何通过扩展 TypeScript
  联合类型和合并对象来扩展 FileType 及 fileTypeExtMap。最后，提供了一个 Vue 与 Element Plus 集成的上传校验示例。
license: true
twikoo: true
footer: false
---
